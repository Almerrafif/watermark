...
main_image_label = tk.Label(center_frame)
main_image_label.pack()
select_main_image_button = tk.Button(center_frame, text="Select Main Image", command=select_main_image, bg="black", fg="white")
select_main_image_button.pack(anchor='center', pady=5)

# Label dan tombol untuk memilih gambar watermark
watermark_image_label = tk.Label(center_frame)
watermark_image_label.pack()
select_watermark_image_button = tk.Button(center_frame, text="Select Watermark Image", command=select_watermark_image, bg="black", fg="white")
select_watermark_image_button.pack(anchor='center', pady=5)

# Tombol untuk menyisipkan watermark
embed_watermark_button = tk.Button(center_frame, text="Embed Watermark", command=embed_watermark, bg="black", fg="white")
embed_watermark_button.pack(anchor='center', pady=5)

# Label untuk menampilkan gambar yang telah disisipi watermark
watermarked_image_label = tk.Label(center_frame)
watermarked_image_label.pack()

# Tombol untuk mendownload hasil gambar
download_button = tk.Button(center_frame, text="Download Watermarked Image", command=download_image, bg="black", fg="white")
