...
test_image_label = tk.Label(center_frame, borderwidth=2, relief="sunken")
test_image_label.pack(pady=5)
select_image_button = tk.Button(button_frame, text="Select Image", command=select_image, bg="black", fg="white")
select_image_button.pack(anchor='center', pady=5)

# Tombol untuk mendeteksi watermark
detect_watermark_button = tk.Button(button_frame, text="Detect Watermark", command=detect_watermark, bg="black", fg="white")
detect_watermark_button.pack(anchor='center', pady=5)

# Label untuk menampilkan hasil deteksi watermark
result_label = tk.Label(center_frame, text="", borderwidth=2, relief="sunken")
result_label.pack(pady=5)

# Label untuk menampilkan gambar watermark yang terdeteksi
extracted_watermark_label = tk.Label(center_frame, borderwidth=2, relief="sunken")
extracted_watermark_label.pack(pady=5)
